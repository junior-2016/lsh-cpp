...
//        //       为了兼顾B方案必须给 Weight Vector 的编号问题给出一个更优的方案,虽然全集很大,但大部分情况下,我们的数据只会利用到
//        //       全集的一部分,并且全集元素的编号不需要固定,只要给每一个不同的元素编一个不同的编号即可,那么完全可以用
//        //       全局HashTable加上Global_Index 来完成,即:
//        //       template<typename ElementType>
//        //       struct GlobalIndexTable {
//        //            static size_t global_index = 0;
//        //            static HashTable<ElementType,size_t> index_table;
//        //            void update(T ele) {
//        //                        if (index_table.find(ele)==index_table.end()) {
//        //                              index_table.insert{ele,global_index++};
//        //                        }
//        //            }
//        //       }
//        //       将这个全局编号哈希表嵌入 WeightMinHash 里面,然后修改更新接口为:
//        //       void update ( HashSet<ElementType> & data){
//        //           Eigen::Sparse_vector weight_vector = construct_weight_vector(data);
//        //           // ... other calculate
//        //       }
//        //       辅助函数: construct_weight_vector(data) {} :
//        //            - 通过提供的 data 集合,在全局编号表里面搜索对应的编号,如果没有相应编号就提供编号;
//        //            - 第二点很重要: 全局编号表的编号每一次扩张一个新编号的时候,就相应的计算和扩充 r_k,ln_ck,beta_k 的 新一列
//        //            - 这里考虑到 r_k,ln_c_k,beta_k,维度(sample,dim), 用行主序稀疏矩阵,每次扩张一列都是顺序扩张,
//        //            这种方式可以进行非常好的优化(如果随机插入值的话就很不好优化了),预先给每一行预留(调用reserve)1000(1000远小于dim)个元素的空间,
//        //            然后用 insert(i,j) 的方法不停添加新的列,每添加新的一列,给每一行预留的空间就少1,除非元素数量超过1000,才会进行新的空间分配,
//        //            需要注意的是每次插完一列后不要调用make_compress()压缩空间,因为预留空间就是留给剩下的元素插入的.
//        //            改进:这里可以采用预读分布采样数据的方法,update()之前, r_k 等矩阵预留 5000(用模板调节) 个元素的空间,
//        //            然后提前插入2500个元素(5000的一半)的随机分布参数,后面update的时候如果出现元素编号>=2500的时候再扩展新的列,
//        //            并且每次扩展直接扩展100行(用模板调整)而非1行.当然这里需要用 distribution_matrix_had_add_num 来记录已经添加的采样数据
//        //            - 最后提供编号生成一个 Eigen 稀疏向量并返回.
//
//        //     TODO: 临时方案,考虑到sparse无法充分发挥dense的各种特性,所以分成两个实现,当 dim <= 100000, 使用 dense 的版本;
//        //                   如果 dim > 100000, 用稀疏矩阵方法.
//
//        //      TODO: C方案: 用 cuda 实现 weight minhash,
//        //       两种方法:
//        //       1.用c++包装开源版本的 minhashcuda;
//        //       2.用Cuda Toolkit 实现. Dense Matrix/vector : CUBLAS/Magma/CULAtools; Sparse Matrix/vector: CUSPARSE/CUSP
//
...
//                    ln_c_k_temp.push_back(std::log(gamma_dis(generator)));  // ln_c_k ~ ln(Gamma(2,1)),Gamma(2,1)不会采样出0,所以ln(Gamma)是安全的
